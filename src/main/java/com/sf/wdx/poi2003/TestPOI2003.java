...
public class TestPOI2003 {
	...
	public void test4(){
		try {
			...
			fieldNames.add("sex");
			List<Cat> list = POIUtils2003.input2List(fieldNames, Cat.class, new FileInputStream(new File("D:/user/80002888/桌面/poi.xls")));
	...
	public void test3(){
		try {
			...
			headers.add("重量");
			headers.add("SEX");
			POIUtils2003.list2Out(headers, cats, new FileOutputStream(new File("D:/user/80002888/桌面/poi.xls")));
			// 转为数组输出
	...
	public void test2(){
		try {
			...
			List<Person> list = POIUtils2003.input2List(fieldNames, Person.class, new FileInputStream(new File("D:/user/80002888/桌面/poi.xls")));
			list.forEach(System.out::println);
		} catch (Exception e) {
			logger.error("get error->", e);
		}
	...
	public void test1() {
		try {
			...
			headers.add("工资");
			// 转为文件输出
			POIUtils2003.list2Out(headers, persons, new FileOutputStream(new File("D:/user/80002888/桌面/poi.xls")));
			// 转为数组输出
